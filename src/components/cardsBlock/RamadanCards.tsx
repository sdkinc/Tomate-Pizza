...
const images = [
	{ id: 'ramadan_variant9', src: '/ramadan9.webp', alt: 'Image 9' },
	{ id: 'ramadan_variant7', src: '/ramadan7.webp', alt: 'Image 7' },
	{ id: 'ramadan_variant2', src: '/ramadan2.webp', alt: 'Image 2' },
	{ id: 'ramadan_variant8', src: '/ramadan8.webp', alt: 'Image 8' },
	{ id: 'ramadan_variant5', src: '/ramadan5.webp', alt: 'Image 5' },
	{ id: 'ramadan_variant3', src: '/ramadan3.webp', alt: 'Image 3' },
];

function RamadanCards(): JSX.Element {
	...
	const [selectedImage, setSelectedImage] = useState('ramadan_variant9');
	...
	const letters = [
		{
			id: 'variant1',
			title: t('ramadanTitleCard.variant1'),
			body: [
				<div className={styles.cards} key="variant1-body">
					<p key="variant1-paragraph1">{t('ramadanCard.variant1.paragraph1')}</p>
					<p key="variant1-paragraph2">{t('ramadanCard.variant1.paragraph2')}</p>
				</div>,
			],
			signature: t('ramadanCardSignature.variant1'),
		},
		{
			id: 'variant2',
			title: t('ramadanTitleCard.variant2'),
			body: [
				<div className={styles.cards} key="variant2-body">
					<p key="variant2-paragraph1">{t('ramadanCard.variant2.paragraph1')}</p>
					<p key="variant2-paragraph2">{t('ramadanCard.variant2.paragraph2')}</p>
				</div>,
			],
			signature: t('ramadanCardSignature.variant2'),
		},
		{
			id: 'variant3',
			title: t('ramadanTitleCard.variant3'),
			body: [
				<div className={styles.cards} key="variant3-body">
					<p key="variant3-paragraph1">{t('ramadanCard.variant3.paragraph1')}</p>
					<p key="variant3-paragraph2">{t('ramadanCard.variant3.paragraph2')}</p>
				</div>,
			],
			signature: t('ramadanCardSignature.variant3'),
		},
	];
	...
	return (
		<div className={styles.cardsBox}>
			<h1 className={styles.titlePage}>{t('ramadanCardsTitle')}</h1>
			<p className={styles.subtitlePage}>{t('ramadanCardsSubtitle')}</p>
			<div className={styles.mainContainer}>
					...
					<div className={styles.previewContainer} ref={previewRef}>
						<div className={styles.letter}>
							<p className={styles.titleLetter}>
								{t(`ramadanTitleCard.${letterVariant}`, { lng: previewLanguage })}
							</p>
							...
							<div className={styles.textContainer}>
								{t(`ramadanCard.${letterVariant}.paragraph1`, {
									lng: previewLanguage,
									name: recipientName || t('defaultRecipientName', { lng: previewLanguage }),
								})}
							</div>
							<div className={styles.textContainerP2}>
								{t(`ramadanCard.${letterVariant}.paragraph2`, {
									lng: previewLanguage,
								})}
							</div>
							<div className={styles.textContainer}>
								<br />
								{t(`ramadanCardSignature.${letterVariant}`, { lng: previewLanguage })}
								<br />
								{senderName}
							</div>
						</div>
				...
				<div className={styles.paragraphBox}>
					<p className={styles.title}>{t('ramadanTitleParagraph')}</p>
					<p className={styles.paragraphSubtitle}>{t('ramadanSubtitle')}</p>

					{Array.from({ length: 9 }, (_, i) => (
						<React.Fragment key={i}>
							<p className={styles.titleParagraph}>{t(`ramadanTextTitleParagraph${i + 2}`)}</p>
							<p className={styles.paragraphs}>{t(`ramadanTextParagraph${i + 2}`)}</p>
						</React.Fragment>
					))}

					<p className={styles.paragraph}>{t('ramadanTextParagraph11')}</p>
					<p
						className={styles.paragraphLast}
						onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
					>
						{t('ramadanTextParagraph12')}
					</p>
				</div>
			</div>
		</div>
